...
import { z } from "zod";
import { MTurkClient, CreateHITCommand, GetHITCommand, ListAssignmentsForHITCommand, 
         GetAccountBalanceCommand, ListHITsCommand, ApproveAssignmentCommand } from "@aws-sdk/client-mturk";
...
const mturkClient = new MTurkClient({ 
  ...
  endpoint: USE_SANDBOX ? "https://mturk-requester-sandbox.us-east-1.amazonaws.com" : undefined
...
server.tool(
  "askHuman",
  {
    ...
    reward: z.string().default("0.05").describe("The reward amount in USD (default: $0.05)"),
    title: z.string().optional().describe("Title for the HIT (optional)"),
    description: z.string().optional().describe("Description for the HIT (optional)"),
    timeoutSeconds: z.number().default(3600).describe("Time until the HIT expires in seconds (default: 1 hour)"),
    maxWaitSeconds: z.number().default(300).describe("Maximum time to wait for a response in seconds (default: 5 minutes)")
  },
  async ({ question, reward, title, description, timeoutSeconds, maxWaitSeconds }) => {
    try {
      ...
      const params = {
        ...
        Description: description || "Please provide your human perspective on this question",
        Question: `
      ...
      while (Date.now() - startTime < maxWaitTime) {
        ...
        if (listAssignmentsResponse.Assignments && listAssignmentsResponse.Assignments.length > 0) {
      ...
      if (assignment && assignment.AssignmentId) {
        ...
        try {
          await mturkClient.send(new ApproveAssignmentCommand({
        ...
        if (assignment.Answer) {
          ...
          const answerText = assignment.Answer
            .replace(/<\?xml.*?\?>/, '')
            .replace(/<Answer>.*?<QuestionIdentifier>.*?<\/QuestionIdentifier>.*?<FreeText>(.*?)<\/FreeText>.*?<\/Answer>/s, '$1');
...
server.tool(
  ...
  async ({ hitId }) => {
    try {
      const hitResponse = await mturkClient.send(new GetHITCommand({ HITId: hitId }));
      ...
      if (assignmentsResponse.Assignments) {
        for (const assignment of assignmentsResponse.Assignments) {
          if (assignment.AssignmentStatus === 'Submitted' && assignment.AssignmentId) {
            try {
              await mturkClient.send(new ApproveAssignmentCommand({
                AssignmentId: assignment.AssignmentId,
                RequesterFeedback: "Thank you for your response!"
              }));
              console.log(`Auto-approved assignment ${assignment.AssignmentId}`);
            } catch (approveError) {
              console.error(`Error auto-approving assignment ${assignment.AssignmentId}:`, approveError);
            }
          }
        }
      }
...
server.resource(
  ...
  async (uri, { info }) => {
    try {
      ...
      switch(info) {
        case "balance":
          const balanceResponse = await mturkClient.send(new GetAccountBalanceCommand({}));
        ...
        case "hits":
          const hitsResponse = await mturkClient.send(new ListHITsCommand({
            MaxResults: 100 // Limit to 100 most recent HITs
          }));
          
          const hitsList = hitsResponse.HITs?.map(hit => ({
            id: hit.HITId,
            title: hit.Title,
            status: hit.HITStatus,
